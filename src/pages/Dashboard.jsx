...
export default function Dashboard({ session }) {
  ...
  const totalBalance = accounts.reduce((sum, acc) => {
    ...
    const expense = transactions
      ...
      .reduce((s, t) => s + t.amount, 0)
    return sum + acc.initial_balance + income - expense
  }, 0)
...
{/* ปุ่มบันทึกรายการ */}
      <button
        ...
        className="w-full bg-blue-600 hover:bg-blue-700 text-white 
                   font-semibold py-3 rounded-xl mb-6 transition"
      >
        + บันทึกรายการ
      </button>
      ...
      <div className="mb-6">
        <div className="flex items-center justify-between mb-3">
          <h2 className="font-semibold text-gray-700">บัญชีของฉัน</h2>
          <button
        ...
        {accounts.length === 0 ? (
          <div className="bg-white rounded-2xl p-8 text-center shadow-sm border border-gray-100">
            ...
            <p className="text-gray-400 text-xs mt-1">กด "+ เพิ่มบัญชี" เพื่อเริ่มต้น</p>
          </div>
        ) : (
            ...
            {accounts.map(acc => {
              ...
              return (
                ...
                >
                  ...
                  <p className="text-xl font-bold">{formatMoney(accBalance)}</p>
                </div>
              )
            })}
          </div>
